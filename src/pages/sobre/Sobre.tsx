import sobreBg from "../../assets/SobrenosBG.jpg";
    ...
    <main className="w-full bg-gradient-to-b from-black via-gray-900 to-black">
      ...
      >
        ...
        <header className="relative z-10 text-center mb-20">
          <h1 className="font-sansita text-6xl md:text-7xl font-bold text-white mb-6">
            Sobre a Vertex.bah
          </h1>
          <div className="mx-auto h-1 w-24 bg-orange-600 rounded-full" />
        </header>

        <section className="relative z-10 max-w-6xl w-full bg-white/95 rounded-3xl shadow-2xl p-10 md:p-14 flex flex-col md:flex-row gap-12 items-center mb-20">
          <img
            src={vertexLogo}
            alt="Vertex.bah"
        ...
        <section className="relative z-10 max-w-6xl w-full mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {valores.map((valor) => (
              <div
                key={valor.titulo}
                className="bg-white/95 rounded-2xl p-8 shadow-xl border border-white/20 hover:-translate-y-1 transition"
              >
        ...
        <section className="relative z-10 max-w-6xl w-full mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[{ title: "Frontend", items: stackFrontend }, { title: "Backend", items: stackBackend }, { title: "Ferramentas", items: ferramentas }].map((group) => (
              <div
                key={group.title}
                className="bg-white/95 rounded-2xl p-8 shadow-xl text-center"
              >
        ...
        <section className="relative z-10 max-w-7xl w-full mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {team.map((member) => (
              <article
                key={member.name}
                className="bg-white/95 rounded-2xl p-8 shadow-xl flex flex-col items-center text-center hover:-translate-y-1 transition"
              >
                <img
                  src={member.img}
                  alt={member.name}
